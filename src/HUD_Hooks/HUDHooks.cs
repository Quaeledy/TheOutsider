        ...
        public static void RainMeter_Update(On.HUD.RainMeter.orig_Update orig, RainMeter self)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < self.circles.Length; i++)
                {
                    ...
                    float num = i / (float)(self.circles.Length - 1);
                    float value = Mathf.InverseLerp(i / (float)self.circles.Length, (i + 1) / (float)self.circles.Length, self.fRain);
                    ...
                    self.circles[i].pos = self.pos + Custom.DegToVec((1f - i / (float)self.circles.Length) * 360f * Custom.SCurve(Mathf.Pow(self.fade, 1.5f - num), 0.6f)) * (self.hud.karmaMeter.Radius + 8.5f + num2 + 4f * self.tickPulse);
                }
            }
        }
