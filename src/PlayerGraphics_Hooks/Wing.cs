    ...
    public class Wing
    {
        ...
        public static void WingPos(PlayerGraphics self, PlayerEx player, Vector2 dif, float bodyRotation, float innerRotation, float outerRotation, int i)
        {
            ...
            if (player.isFlying)
                ...
                wingW = Mathf.Lerp(0f, self.player.playerState.isPup ? 8f : 5f, Mathf.Abs(Mathf.Sin(bodyRotation * 2f)));
            else if (self.player.input[0].x < 0)
                wingW = -Mathf.Lerp(0f, self.player.playerState.isPup ? 8f : 5f, Mathf.Abs(Mathf.Sin(bodyRotation * 2f)));
            else
                wingW = 0;
            ...
            else
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (j != 1)
                    {
                        ...
                        //爬行姿态调整
                        player.wing[startNum + 2].pos = Vector2.Lerp(player.wing[startNum + 2].pos, player.wing[startNum + 0].pos, 0.5f * Mathf.Abs((1 - Mathf.Cos(bodyRotation))) / 2);
                        player.wing[startNum + 3].pos = Vector2.Lerp(player.wing[startNum + 3].pos, player.wing[startNum + 2].pos, 0.75f * Mathf.Abs((1 - Mathf.Cos(bodyRotation))) / 2);
                    }
                    //外层翅膀
                    else
                    {
                        ...
                        //爬行姿态调整
                        player.wing[startNum + 2].pos = Vector2.Lerp(player.wing[startNum + 2].pos, player.wing[startNum + 0].pos, 0.5f * Mathf.Abs((1 - Mathf.Cos(bodyRotation))) / 2);
                        player.wing[startNum + 3].pos = Vector2.Lerp(player.wing[startNum + 3].pos, player.wing[startNum + 5].pos, 0.5f * Mathf.Abs((1 - Mathf.Cos(bodyRotation))) / 2);
                    }
                }
            }
        }
        ...
        public static void WingLevel(PlayerEx player, RoomCamera.SpriteLeaser sLeaser, float bodyRotation)
        {
            ...
            //设置图层
            //俯冲
            if (player.isFlying && bodyRotation < -1.8f || bodyRotation > 1.8f)
            {
            ...
            //侧飞
            else if (player.isFlying && (bodyRotation < -0.5f || (bodyRotation < -0.3f && player.flutterTimeAdd <= player.upFlightTime)))
            {
            ...
            //倒立
            else if (!player.isFlying && bodyRotation < -2.35f || bodyRotation > 2.35f)
            {
                sLeaser.sprites[player.WingSprite(1, 1)].MoveInFrontOfOtherNode(sLeaser.sprites[9]);
                sLeaser.sprites[player.WingSprite(1, 0)].MoveInFrontOfOtherNode(sLeaser.sprites[player.WingSprite(1, 1)]);
                for (int i = 0; i < 2; i++)
            ...
            //侧身
            else if (!player.isFlying && bodyRotation < -0.12f)
            {
                sLeaser.sprites[player.WingSprite(0, 1)].MoveInFrontOfOtherNode(sLeaser.sprites[9]);
                sLeaser.sprites[player.WingSprite(0, 0)].MoveInFrontOfOtherNode(sLeaser.sprites[player.WingSprite(0, 1)]);
                sLeaser.sprites[player.WingSprite(0, 2)].MoveInFrontOfOtherNode(sLeaser.sprites[player.WingSprite(0, 0)]);
        ...
        private static void PlayerGraphics_DrawSprites(On.PlayerGraphics.orig_DrawSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            if (PlayerHooks.PlayerData.TryGetValue(self.player, out var player))
            {
                ...
                {
                    //内层翅膀渐变颜色
                    sLeaser.sprites[player.WingSprite(i, 2)].color = player.GetSpeckleColor();
                    //外层翅膀颜色与不透明度
                    sLeaser.sprites[player.WingSprite(i, 1)].color = player.GetLepidoticWingColor();
