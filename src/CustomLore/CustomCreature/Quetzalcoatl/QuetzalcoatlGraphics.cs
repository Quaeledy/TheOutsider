...
namespace TheOutsider.CustomLore.CustomCreature.Quetzalcoatl
{
     ...
     : GraphicsModule
    {
        ...
        public QuetzalcoatlGraphics(Quetzalcoatl quetzalcoatl) : base(quetzalcoatl, false)
        {
            quet = quetzalcoatl;

            var state = Random.state;
            Random.InitState(quetzalcoatl.abstractCreature.ID.RandomSeed);
        ...
        public override void Reset()
        {
            ...
            {
                body[i, 0] = quet.firstChunk.pos - dir * i;
                body[i, 1] = body[i, 0];
                body[i, 2] *= 0f;
            }
        }

        // TODO make dead quetzalcoatl body flop
        public override void Update()
        {
            ...
            if (quet.Consious && quet.grasps[0] == null)
            {
            ...
            for (int i = 0; i < body.GetLength(0); i++)
            {
                body[i, 1] = body[i, 0];
                body[i, 0] += body[i, 2];
                body[i, 2] *= quet.airFriction;
                body[i, 2].y -= quet.gravity;
                body[i, 2] += quet.bloat * -quet.needleDir * 3f;
            }

            for (int j = 0; j < body.GetLength(0); j++)
            {
                SharedPhysics.TerrainCollisionData terrainCollisionData = new(body[j, 0], body[j, 1], body[j, 2], (2.5f - j * 0.5f) * sizeFac, default, quet.firstChunk.goThroughFloors);
                terrainCollisionData = SharedPhysics.VerticalCollision(quet.room, terrainCollisionData);
                terrainCollisionData = SharedPhysics.HorizontalCollision(quet.room, terrainCollisionData);
                terrainCollisionData = SharedPhysics.SlopesVertically(quet.room, terrainCollisionData);
                ...
                if (j == 0)
                {
                    Vector2 a = Custom.DirVec(body[j, 0], quet.firstChunk.pos) * (Vector2.Distance(body[j, 0], quet.firstChunk.pos) - 5f * sizeFac);
                    body[j, 0] += a;
                    body[j, 2] += a;
                }
                else
            ...
            float d = Mathf.Pow(Mathf.InverseLerp(0.25f, -0.75f, Vector2.Dot((quet.firstChunk.pos - body[0, 0]).normalized, (body[0, 0] - body[1, 0]).normalized)), 2f);
            body[1, 2] -= Custom.DirVec(body[1, 0], quet.firstChunk.pos) * d * 3f * sizeFac;
            body[1, 0] -= Custom.DirVec(body[1, 0], quet.firstChunk.pos) * d * 3f * sizeFac;

            quet.needleDir = (quet.needleDir + Custom.DirVec(body[0, 0], quet.firstChunk.pos) * 0.2f).normalized;
            ...
            for (int k = 0; k < squirm.GetLength(0); k++)
            {
                ...
                squirm[k, 0] = Mathf.Sin(squirmOffset + k * Mathf.Lerp(0.5f, 2f, squirmWidth)) * squirmAmp * (1f - quet.bloat);
            }
        }
        /*
        private void UpdateSounds()
        {
            if (soundLoop == null && quet.Consious)
            {
                soundLoop = quet.room.PlaySound(SoundID.Cicada_Wings_LOOP, quet.firstChunk, true, 1f, 1f);
                soundLoop.requireActiveUpkeep = true;
            }
            else if (soundLoop != null && (!quet.Consious || quet.room != soundLoop.room || soundLoop.slatedForDeletetion))
            {
        ...
        */
        ...
        public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            Vector2 chk0Pos = Vector2.Lerp(quet.firstChunk.lastPos, quet.firstChunk.pos, timeStacker);
            Vector2 bodyPos = Vector2.Lerp(body[0, 1], body[0, 0], timeStacker);
            Vector2 headPos = Vector2.Lerp(body[1, 1], body[1, 0], timeStacker);
            Vector2 segmentDir = -Vector3.Slerp(quet.lastNeedleDir, quet.needleDir, timeStacker);
            ...
            if (quet.room != null)
            {
                ...
                darkness = quet.room.DarknessOfPoint(rCam, bodyPos);
            ...
            headPos += chkDir * (7f * (1f - quet.bloat)) * sizeFac;
            ...
            for (int m = 0; m < 2; m++)
            {
                Vector2 firstChunkPos = Vector2.Lerp(quet.firstChunk.lastPos, quet.firstChunk.pos, timeStacker);
        ...
        public override void ApplyPalette(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, RoomPalette palette)
        {
            ...
            for (int i = 0; i < m[0].verticeColors.Length; i++)
            {
                ...
                m[0].verticeColors[i] = Color.Lerp(yellow, to, 0.25f + Mathf.InverseLerp(0f, 0.2f + 0.8f, value) * 0.75f * quet.bloat);
            }
