...
namespace TheOutsider.CustomLore.CustomCreature
{
    sealed class Quetzalcoatl : Creature, IFlyingCreature
    {
        ...
        public Quetzalcoatl(AbstractCreature acrit) : base(acrit, acrit.world)
        {
        ...
        {
            graphicsModule ??= new QuetzalcoatlGraphics(this);
        ...
        void Act()
        {
            ...
            else
            {
                ...
                {
                    MoveTowards(room.MiddleOfTile(lastFollowedConnection?.DestTile ?? Room.StaticGetTilePosition(followingPos)));
                }
                if (Submersion > .5)
                {
        ...
        public int FoodPoints => (int)Mathf.Lerp(20f, 30f, bloat);/*
